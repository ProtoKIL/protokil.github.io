...
산악 환경에서 위험 요소와 사람을 탐지하기 위해 복수 센서와 이동 플랫폼을 통합한 탐지 로봇 프로젝트입니다.

## 간단한 설명

사람이 접근하기 어려운 산악 지형에서 탐지와 맵핑을 수행해 구조 활동을 보조하는 로봇 시스템 프로젝트입니다.
...
| Team | 6 members |
| Role | Robotics System Design · Presentation · System Integration |
...
LiDAR 맵핑, 깊이 정보와 열화상 탐지의 역할을 구분하고 무한궤도형 이동 플랫폼에 센서를 통합했습니다. 설계 단계에서 모든 부품을 미리 어셈블리해 크기를 줄였으며, LiDAR 장착부에는 거북이 등껍질을 닮은 커버를 적용해 전자 부품의 방수성을 고려했습니다.

## System Architecture

1. Mobile Platform / Tracked Drive — 비정형 지형 이동
2. LiDAR / Scanning & Mapping — 주변 지형 맵핑
...
LiDAR 스캐닝과 맵핑을 구현했으며, 완전한 SLAM을 위해서는 엔코더 모터 등의 추가 하드웨어가 필요했습니다. AI Camera는 서보모터와 연계해 인체 추적과 장애물 감지를 구현했습니다. Thermal Camera와 AI Camera의 연동에는 실패했지만 열원 감지 테스트는 완료했습니다. GPS Module로 실시간 위치 추적과 핀포인트 시스템을 구축했으며, LoRa Module은 다중 로봇 관제 시스템의 기반으로 구성했으나 별도 검증은 진행하지 않았습니다.

하드웨어 설계와 통합, 배선이 주 과제였습니다. 컴퓨터공학과·자동차공학과 팀원들과 협업하며 원활한 소통, 함께 작업하는 시간과 공간이 결과물의 완성도에 미치는 영향을 배웠습니다.
...
3. Center & Distance / Align Logic — 중심·거리 좌표 계산
...
- Camera Detection — 카메라 기반 농구공 탐지 영상
- Demo 1 — 주행 및 슈팅 시연 1
- Demo 2 — 주행 및 슈팅 시연 2
- Demo 3 — 주행 및 슈팅 시연 3
- Result — 농구 슈팅 결과 영상
...
PETG 브라켓과 ABS 툴헤드·핫엔드 마운트를 직접 설계하고, 전장·냉각·Klipper 제어를 통합한 델타 3D 프린터입니다.
...
2. Heated Bed / Ø310 mm — 출력면 가열·지지
...
VOCs를 ppb 수준에서 검출하기 위한 Photoacoustic Gas Sensor 연구에 참여했습니다.
센서 구조와 함께 Pre-amplifier, Band-pass Filter, Lock-in Amplifier 기반 신호 처리 회로를 다뤘습니다.
...
산악 환경에서의 탐지와 구조 활동 보조를 목표로 LiDAR, AI Depth Camera, 열화상 카메라를 결합한 자율주행 로봇을 개발했습니다.
ROS 기반 맵핑과 주행 시스템을 구성하고, 센서별 역할 정의와 하드웨어·소프트웨어 통합을 수행했습니다.
